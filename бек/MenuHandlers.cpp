...
void showCatalogFlow() {
    ...
    if (count == 0) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            cout << "   Technology:  " << list[i].technology << endl; 
            cout << "   Care:        " << list[i].careInstructions << endl; 
...
void addProductFlow() {
    ...
    if (saveProduct(p)) {
        cout << "[Успіх] Товар збережено!" << endl;
    }
}
...
void viewCartFlow() {
    ...
    if (count == 0) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (p.id != -1) {
                cout << i + 1 << ". " << p.modelName 
                     << " (" << p.shortDescription << ")"
                     << " | Ціна: $" << cart[i].priceAtPurchase << endl;
            } else {
                cout << i + 1 << ". [Товар не знайдено в базі] ID: " << cart[i].productId 
                     << " | Ціна: $" << cart[i].priceAtPurchase << endl;
            }
