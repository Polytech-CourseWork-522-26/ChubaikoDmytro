...
bool saveProduct(const Product& product) { 
    ...
    file.write((const char*)&product, sizeof(Product));
    file.close();
    return true;
}
...
bool updateProductFull(const Product& updatedProduct) {
    ...
    while (file.read((char*)&temp, sizeof(Product))) {
        if (temp.id == updatedProduct.id) {
            // Повертаємо курсор на початок цього запису
            int pos = (int)file.tellg() - sizeof(Product);
            file.seekp(pos);
            
            // Записуємо оновлену структуру
            file.write((const char*)&updatedProduct, sizeof(Product));
            file.close();
            return true;
        }
    }
